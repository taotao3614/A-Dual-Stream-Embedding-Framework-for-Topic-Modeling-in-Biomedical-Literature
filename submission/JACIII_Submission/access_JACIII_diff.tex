...
{\color{red}Despite these advances, existing approaches exhibit several limitations that motivate the present study:}
...
{\color{red}An overview of the proposed topic modelling framework is illustrated in \textbf{Fig.~1}.} The architecture comprises five modular stages: data preparation, dual-stream embedding, topic modelling with BERTopic, topic labelling via a large language model (LLM), and evaluation and analysis.
...
\noindent{\color{red}\textbf{First-layer fusion: intra-MeSH term weighting ($\boldsymbol{\beta}$).}
...
To model latent thematic structures in the biomedical literature, we adopted the BERTopic framework~\cite{grootendorst_bertopic_2022}, a modular and extensible pipeline that integrates state-of-the-art sentence embeddings with clustering and keyword extraction. {\color{red}BERTopic is particularly well suited for biomedical domain applications because of its ability to generate interpretable topics from high-dimensional contextual embeddings via a structured and visually interpretable workflow (see \textbf{Fig.~\ref{fig:bertopic_pipeline}}).}
...
{\color{red}The BERTopic pipeline, illustrated in \textbf{Fig.~\ref{fig:bertopic_pipeline}}, consists of the following key components:}
...
{\color{red}In Equation~(\ref{eq:tc}), the topic coherence (TC) score quantifies the semantic consistency among the top-ranked keywords of a given topic.} Specifically:
...
{\color{red}Equation~(\ref{eq:tc}) averages the pairwise NPMI scores across all unique word pairs in a topic, yielding a coherence score in the range $[-1, 1]$, where higher values indicate stronger semantic associations and better topic consistency.}
...
{\color{red}In Equation~(\ref{eq:td}), the topic diversity (TD) score measures how distinct the topics are by evaluating vocabulary overlap:}
...
{\color{red}\textbf{Figure~\ref{fig:tc_heatmap}} shows the distribution of topic coherence (TC) across different $\alpha$--$\beta$ combinations.} The highest TC value of 0.6172 was achieved at $\alpha=0.40$ and $\beta=0.65$, indicating that emphasizing textual information alongside strongly weighted major MeSH terms leads to more semantically consistent topic clusters.
...
{\color{red}\textbf{Figure~\ref{fig:td_heatmap}} illustrates the variation in topic diversity (TD).} The highest TD score of 0.7458 occurred at $\alpha=0.45$ and $\beta=0.40$, suggesting that balanced weighting between textual and MeSH inputs fosters broader thematic coverage and reduced keyword overlap across topics.
...
{\color{red}\textbf{Figure~\ref{fig:combined_heatmap}} presents the combined TC+TD scores, reflecting the trade-off between intratopic coherence and intertopic diversity.} The optimal joint performance (TC+TD = 1.3443) was also observed at $\alpha=0.45$ and $\beta=0.40$, confirming the robustness of this fusion configuration.
...
The dual-stream embedding topic modelling framework demonstrated significant improvements over baseline models across all evaluated metrics. {\color{red}The detailed performance of each model is summarized clearly in \textbf{Table~\ref{tab:model_performance}}.}
...
{\color{red}\textbf{Table~\ref{tab:topic_labels}} illustrates five representative examples of LLM-generated titles.} Each title corresponds to a unique topic and demonstrates how natural language summarization can bridge the gap between machine-generated keywords and human comprehension. The results show that LLM-based postprocessing significantly improves topic accessibility, reducing the cognitive effort needed to interpret and organize thematic outputs.
...
The dual-stream design provides two key benefits. {\color{red}First, it enables semantic complementarity between contextual embeddings, which capture linguistic nuance and discourse structure, and MeSH descriptors, which reflect domain-specific expert annotation.} {\color{red}Second, the parameterized weighted fusion mechanism introduces fine-grained control over the contribution of each information type.} {\color{red}The grid search showed that moderate $\alpha$ and $\beta$ values yielded the best performance, underscoring the need for balanced integration rather than dominance of either stream.}
...
This subsection provides a methodological comparison of our dual-stream embedding framework with existing topic modeling approaches. {\color{red}\textbf{Table~\ref{tab:comparison}} summarizes the key characteristics and design choices of related studies alongside our proposed method.}
